...
1. Remove variables that are not useful for prediction, e.g., *user_name*, timestamps and direct measurements.
...
To make an initial selection of features I computed a correlation matrix. The amount of variables makes it difficult to visualize all relationships or to compare them manually. For these reasons I made a function that iterates over all variables discarding those whose correlation factor with an already selected variable equals or exceeds a given threshold.
...
The following script makes use of the previous support functions to tidy up the dataset and perform an initial selection of covariates. 
...
The resulting list of covariates is still long. Plotting and visual inspection of the variables does not provide clear hints about which variables would be the more appropriate to construct the model. It is difficult to identify clear patterns that could lead to candidates for the model. At this point I decide to leave the heavy lifting to the Caret package.

**TODO: Afegir figura on es vegi algun patró**
...
To create the prediction model I take advantage of the capabilities of the `train()` function. In particular, I use the `trainControl()` function to specify how cross validation will be performed. In the code snippet below, the configuration is defined to perform 10-fold cross-validation.
...
Now we let Caret do the training using random forests and the specified configuration parameters. The features that will be considered are those obtained after evaluating the correlation matrix (see **Feature selection** section above).
...
To check how the selection of certain parameters affects the resulting model, some other options were tested. The table below shows the `Accuracy` of four computed models. Column labels indicate what threshold was used for the correlation factor used to choose variables (in parenthesis the number of variables). Row labels indicate which value was used for k-folding.
...
Before testing the model with the test data, we can evaluate the model with the remaining values from our previous partitioning (which was done due to performance considerations). We find out that accuracy is very close to the one reached by the model.
...
cm <- confusionMatrix(pred, clean_t$classe)
...
The main problem encountered while doing the project was understanding the structure of the input data. Rows that seem to contain instant measurements are combined with rows that contain aggregated values, such as averages, minimums or maximums, among other more complex functions. Some columns only make sense for the latter, which means there are lots of empty fields. This was confusing and my inclination was to believe that the relevant rows were the ones with aggregate values, which somehow would summarize and characterize the performance of the participants during the whole duration of one run of the exercise. It wasn't until I had a look at the test data that I realized I was doing it wrong.

The second problem was selecting the variables, there are many of them and manual approaches and visual inspection of many plots would not help. And, finally, choosing a training method and its related parameters was not an easy decision either. The **Practical Machine Learning** course of the **Data Science Specialization** covers a lot of material rather superficially. Although I researched other resources to learn more about the topic, in the end my decisions were based on trial and error than on solid arguments.
